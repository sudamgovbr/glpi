...
if (isset($_GET['create_ticket'])) {
  //INICIO MODULO PARA TRAVAR CRIACAO DE CHAMADO	
	if(!Ticket::ticketCountVerify(Ticket::ticketIdSatisfaction())){
        Html::redirect($CFG_GLPI['root_doc'].'/front/ticket.form.php?id='.Ticket::ticketIdSatisfaction().'&forcetab=Ticket$3');
	//header('Location: '.'http://suporte.sudam.intra/glpi/front/ticket.form.php?id='.Ticket::ticketIdSatisfaction().'&forcetab=Ticket$3');
	$message = 'Por favor preencha a avaliacao do atendimento antes de abrir um novo chamado';
