...
class Ticket extends CommonITILObject {
   ...
   **/
   function showFormHelpdesk($ID, $ticket_template = false) {
      ...
      echo "<tr class='tab_bg_1'><th class='center' colspan='2'>";
      echo "Para falar com a Central de Serviços da CTI, ligue no telefone : 4008-5554";
      echo "</th></tr>"; 
     
      echo "<tr class='tab_bg_1'><th class='center' colspan='2'>";
      echo "Para criar uma Requisição de Serviço (REQUISIÇÃO)";
      echo "</th></tr>";

      echo "<tr class='tab_bg_1'><th class='center' colspan='2'>";
      echo "Para reportar uma falha/erro/indisponibilidade/mau funcionamento (INCIDENTE)";
      echo "</th></tr>"; 
...
//INICIO MODULO PARA TRAVAR CRIACAO DE CHAMADO   
   static function ticketIdSatisfaction(){
      global $DB;
...
static function ticketCountVerify($result){  
if($result == null){
return true;
}else{
return false;
...
static function getTaskState($id_ticket){
        global $DB;
        $query = "SELECT `glpi_tickettasks`.`state`
                  FROM `glpi_tickettasks`
                  WHERE `glpi_tickettasks`.`tickets_id` = $id_ticket";
	$result  = $DB->query($query);
...
//FIM MODULO PARA TRAVAR CRIACAO DE CHAMADO
}
